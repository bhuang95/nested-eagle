...
def open_dataset(t0: pd.Timestamp):
    ...
    data_dir = "/pscratch/sd/t/timothys/nested-eagle/0.25deg-06km/production/gfs-hrrr/stage1c/inference-precip-validation"
    xds = xr.open_dataset(
        f"{data_dir}/{st0}.48h.nc",
        decode_timedelta=True,
        chunks="auto",
    )
    xds = xds.rename({"values": "cell"})
    xds["cell"] = xr.DataArray(
        np.arange(len(xds.cell)),
        dims=("cell",),
    )
    xds = xds.isel(cell=slice(_n_y*_n_x))
...
if __name__ == "__main__":
    ...
    store_path = "/pscratch/sd/t/timothys/nested-eagle/0.25deg-06km/production/gfs-hrrr/stage1c/inference-precip-validation/nested-eagle.conus06km.precip.zarr"

    all_t0 = pd.date_range("2023-02-01T06", "2024-01-29T18", freq="6h")
    ...
    print(f"Created Container at {store_path}")
